...
def summarize_mean_file(files: list):
    ...
    for file in tqdm.tqdm(files):
        try:
            ...
            summary = df.reset_index().rename(columns={'index':'timestamp'}).groupby
            print(summary)
            summary['site_id'] = df['site_id'][0]
            summary['site_name'] = df['site_name'][0]
            summary['year'] = df['year'][0]
...
if __name__ == '__main__':
    ...
    summarize_mean_file(files).to_csv('CPCB_Issues/AirPy_v2/new_data/summary/summary_mean_monthly.csv', index=False)
